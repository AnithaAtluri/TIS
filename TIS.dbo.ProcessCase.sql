...
/****** Object:  StoredProcedure [dbo].[ProcessCase]    Script Date: 2/21/2017 8:35:17 AM ******/
SET ANSI_NULLS ON
GO
...
AS 
--*************************************************************************************************
  --Modification History : passing additional parameter LastUserID , Modified Update Case statement to include the
  --					    LastUserID,UpdateDateTime
...
SELECT @OdysseyAPIMessage=Doc FROM @XmlDocument
	...
	IF CHARINDEX('ERRORSTREAM',CONVERT(NVARCHAR(MAX),@OdysseyAPIResponse))=0
		BEGIN
			...
			Update [Case]
			SET CaseNumber=(SELECT OdysseyAPIResponse.value('(/TxnResponse/Result[5]/CaseNumber)[1]','nVarChar(50)') FROM OdysseyMessageTable WHERE (CaseID=@CaseID)), FilingStatusCD=9
			,LastUserID = @UserID,UpdateDateTime=CURRENT_TIMESTAMP
			WHERE (CaseID=@CaseID)

			--EXEC CaseEventInsertUpdate @CaseID=@CaseID,@CaseEventTypeID=9,@EventDate=@EventDate,@LastUserID=2
			EXEC CaseEventInsertUpdate @CaseID=@CaseID,@CaseEventTypeID=9,@EventDate=@EventDate,@LastUserID=@UserID
	...
	ELSE
		BEGIN
			...
			--WHERE (CaseID=@CaseID)
			UPDATE [Case]
			SET FilingStatusCD=8,LastUserID = @UserID,UpdateDateTime=CURRENT_TIMESTAMP
			WHERE (CaseID=@CaseID)
			--EXEC CaseEventInsertUpdate @CaseID=@CaseID,@CaseEventTypeID=8,@EventDate=@EventDate,@LastUserID=2
			EXEC CaseEventInsertUpdate @CaseID=@CaseID,@CaseEventTypeID=8,@EventDate=@EventDate,@LastUserID=@UserID
		END
